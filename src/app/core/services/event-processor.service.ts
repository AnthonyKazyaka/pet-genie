 ...
 * CRITICAL: All regex patterns preserved from gps-admin EventProcessor
 */
@Injectable({
  providedIn: 'root',
})
export class EventProcessorService {
  ...
  private readonly workEventPatterns = {
  ...
  isDefinitelyPersonal(title: string): boolean {
    for (const pattern of Object.values(this.personalEventPatterns)) {
  ...
  isOvernightEvent(event: CalendarEvent): boolean {
    ...
    // Check by pattern
    if (
      this.workEventPatterns.housesitSuffix.test(title) ||
      this.workEventPatterns.overnight.test(title)
    ) {
      return true;
    }
  ...
  extractClientName(title: string): string {
    ...
    const match = title.match(this.workEventPatterns.petNamePattern);
    if (match) {
      return match[1].trim();
    }
  ...
  extractServiceInfo(title: string): ServiceInfo {
    ...
    } else if (this.workEventPatterns.housesitSuffix.test(title)) {
      info.type = 'housesit';
      info.duration = 1440; // 24 hours
    } else if (this.workEventPatterns.overnight.test(title)) {
      info.type = 'overnight';
      info.duration = 720; // 12 hours
    } else if (this.workEventPatterns.nailTrim.test(title)) {
      info.type = 'nail-trim';
      info.duration = info.duration || 15;
    } else if (this.workEventPatterns.walk.test(title)) {
      info.type = 'walk';
    } else if (this.workEventPatterns.dropIn.test(title)) {
      info.type = 'drop-in';
